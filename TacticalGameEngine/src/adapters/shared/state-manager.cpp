...
namespace Engine::Adapters::Shared
{
	StateManager::StateManager(std::shared_ptr<Core::Interfaces::ILogger> logger,
							   ...
							   std::shared_ptr<Core::Interfaces::IProjectRepository> repo) : logger_(logger),
																							 ...
																							 current_state_(AppState::Editor)
	{
		...
		editor_menu_->registerAction(std::make_unique<Editor::Actions::RunSimulationAction>(
			logger,
			[this]()
			{
				this->changeState(AppState::Runtime);
			}));
		editor_menu_->registerAction(std::make_unique<Editor::Actions::ExitAppAction>(
			logger,
			[this]()
			{
				this->changeState(AppState::Exit);
			}));
